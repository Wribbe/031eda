...
std::vector <Article> &MemoryDatabase::load_articles(ID ng_id) {
    return database[ng_id].get_articles();
}
...
bool MemoryDatabase::newsgroup_exists(ID ng_id) {
    if (database.find(ng_id) != database.end()) { return true; }
    else { return false; }
}

bool MemoryDatabase::article_exists(ID ng_id, ID a_id) {
    return database[ng_id].article_exists(a_id);
}
